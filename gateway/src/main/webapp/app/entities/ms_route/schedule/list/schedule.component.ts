...
export class ScheduleComponent implements OnInit {
  ...
  trackId = (item: ISchedule): string => this.scheduleService.getScheduleIdentifier(item);
  ...
  search(query: string): void {
    ...
    if (query && predicate && ScheduleComponent.NOT_SORTABLE_FIELDS_AFTER_SEARCH.includes(predicate)) {
  ...
  protected fillComponentAttributeFromRoute(params: ParamMap, data: Data): void {
    ...
    if (params.has('search') && params.get('search') !== '') {
      ...
      const { predicate } = this.sortState();
      if (predicate && ScheduleComponent.NOT_SORTABLE_FIELDS_AFTER_SEARCH.includes(predicate)) {
        this.sortState.set({});
      }
    }
  }
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.schedules.set(dataFromBody);
  }

  protected fillComponentAttributesFromResponseBody(data: ISchedule[] | null): ISchedule[] {
    // If there is previous link, data is a infinite scroll pagination content.
    if (this.links().prev) {
      const schedulesNew = this.schedules();
      if (data) {
        for (const d of data) {
          if (schedulesNew.some(op => op.id === d.id)) {
            schedulesNew.push(d);
          }
        }
      }
  ...
  protected queryBackend(): Observable<EntityArrayResponseType> {
    ...
    if (this.currentSearch && this.currentSearch !== '') {
      return this.scheduleService.search(queryObject).pipe(tap(() => (this.isLoading = false)));
    }
