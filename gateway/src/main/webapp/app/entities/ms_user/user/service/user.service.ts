...
type RestOf<T extends IUser | NewUser> = Omit<T, 'dateOfBirth' | 'createdAt' | 'updatedAt'> & {
...
export class UserService {
  ...
  protected convertDateFromClient<T extends IUser | NewUser | PartialUpdateUser>(user: T): RestOf<T> {
    return {
      ...user,
      dateOfBirth: user.dateOfBirth?.format(DATE_FORMAT) ?? null,
  ...
  protected convertDateFromServer(restUser: RestUser): IUser {
    return {
      ...restUser,
      dateOfBirth: restUser.dateOfBirth ? dayjs(restUser.dateOfBirth) : undefined,
