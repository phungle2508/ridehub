...
type RestOf<T extends IBooking | NewBooking> = Omit<T, 'createdAt' | 'updatedAt' | 'expiresAt'> & {
...
export class BookingService {
  ...
  protected resourceUrl = this.applicationConfigService.getEndpointFor('api/bookings', 'ms_booking');
  ...
  protected convertDateFromClient<T extends IBooking | NewBooking | PartialUpdateBooking>(booking: T): RestOf<T> {
    return {
      ...
      updatedAt: booking.updatedAt?.toJSON() ?? null,
      expiresAt: booking.expiresAt?.toJSON() ?? null,
    };
  }
  ...
  protected convertDateFromServer(restBooking: RestBooking): IBooking {
    return {
      ...
      updatedAt: restBooking.updatedAt ? dayjs(restBooking.updatedAt) : undefined,
      expiresAt: restBooking.expiresAt ? dayjs(restBooking.expiresAt) : undefined,
    };
  }
