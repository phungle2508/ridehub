...
SHARED_IMPORT="optional:file:/home/phungvip/Desktop/work-space/ticket-store/backend/ticket-store-central-config/application-dev.yml"
...
for ms in "${!DB_PORTS[@]}"; do
  ...
  if [[ -f "$yml" ]]; then
    ...
    # Update spring.config.import
    if grep -q "spring:" "$yml"; then
      # Replace existing import line
      sed -i "s|import:.*|import: $SHARED_IMPORT|" "$yml"
    else
